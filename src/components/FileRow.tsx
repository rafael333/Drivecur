...
import { FOLDER_COLOR_OPTIONS, getFolderColor, setFolderColor, removeFolderColor, isFolderPinned, togglePinFolder, isFolderFavorite, toggleFavoriteFolder, FILE_COLOR_OPTIONS, getFileTextColor, setFileTextColor, removeFileTextColor, isFileFavorite, toggleFavoriteFile } from '../lib/savedFolders';
...
export function FileRow({ file, isSelected, onClick, onDoubleClick, isEven, hasAnnotations, onColorChange, accessToken, onDownloadStart, onDownloadProgress, onDownloadComplete, onDownloadError }: FileRowProps) {
  ...
  const isPinned = isFolder && file.id ? isFolderPinned(file.id) : false;
  const isFavorite = file.id ? (isFolder ? isFolderFavorite(file.id) : isFileFavorite(file.id)) : false;
