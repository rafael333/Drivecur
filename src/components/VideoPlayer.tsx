...
export function VideoPlayer({ file, accessToken }: VideoPlayerProps) {
  ...
  useEffect(() => {
    ...
    // Define a velocidade de reprodução inicial
  ...
  // Atualiza a velocidade quando muda
  useEffect(() => {
    const video = videoRef.current;
    if (video) {
      video.playbackRate = playbackRate;
      ...
      >
        ...
        <div className="px-2 sm:px-4 py-2 sm:py-3 flex items-center gap-2 sm:gap-4 flex-wrap overflow-visible">
          ...
          <div className="relative hidden sm:block" ref={speedMenuRef}>
            ...
            {showSpeedMenu && (
              <div className="absolute bottom-full right-0 mb-2 bg-[#1a1a1a] border border-gray-800 rounded-lg shadow-xl overflow-hidden min-w-[8rem] max-w-[90vw]">
                {speedOptions.map((speed) => (
                  <button
                    key={speed}
                    onClick={() => handleSpeedChange(speed)}
                    className={`w-full px-4 py-2.5 sm:py-2 text-left text-sm transition-colors touch-manipulation ${
                      playbackRate === speed
        ...
        <div className="px-2 sm:px-4 pb-2 sm:hidden flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={speedMenuRef}>
              ...
              {showSpeedMenu && (
                <div className="absolute bottom-full right-0 mb-2 bg-[#1a1a1a] border border-gray-800 rounded-lg shadow-xl overflow-hidden min-w-[7rem] max-w-[90vw]">
                  {speedOptions.map((speed) => (
                    <button
                      key={speed}
                      onClick={() => handleSpeedChange(speed)}
