...
import { ArrowLeft, Star, Users, MessageSquare, Palette, X, Pin, Download, FolderDown, ArrowUpDown, ArrowUp, ArrowDown, ChevronRight, Home } from 'lucide-react';
import { FileIcon } from './FileIcon';
import { getFolderColor, setFolderColor, removeFolderColor, isFolderPinnedSync, togglePinFolder, isFolderFavorite, isFolderFavoriteSync, toggleFavoriteFolder, getFavoritedFolders, FavoritedFolder, FILE_COLOR_OPTIONS, getFileTextColor, setFileTextColor, removeFileTextColor, isFileFavorite, toggleFavoriteFile } from '../lib/savedFolders';
...
export function FileList({ selectedFile, setSelectedFile, searchQuery, accessToken, onViewFile, viewMode = 'my-drive', onDownloadStart, onDownloadProgress, onDownloadComplete, onDownloadError }: FileListProps) {
  ...
  useEffect(() => {
    const handleOpenSavedFolder = async (event: CustomEvent) => {
      ...
      if (folderResponse.ok) {
        ...
        if (folderData.name) {
          ...
          setSelectedFile(null as unknown as FileItem);
        }
      } else {
    ...
    window.addEventListener('openSavedFolder', handleOpenSavedFolder as unknown as EventListener);
    return () => {
      window.removeEventListener('openSavedFolder', handleOpenSavedFolder as unknown as EventListener);
    };
    ...
    <div className="flex-1 flex flex-col overflow-hidden animate-fade-in">
      <div className="px-3 sm:px-6 py-4 sm:py-6">
        ...
        <div className="flex items-center gap-2 mb-2">
          {/* Back Button (Only if in a folder) */}
          {folderPath.length > 0 && (
            <button
              onClick={() => handleFolderBack()}
              className="p-2 sm:p-2.5 bg-app-surface/50 border border-app-glassBorder hover:bg-app-glassHover rounded-xl transition-all active:scale-95 group shadow-sm mr-2"
              title={`Voltar para ${folderPath.length > 1 ? folderPath[folderPath.length - 2].name : 'Início'}`}
            >
          ...
          {/* Breadcrumbs Track */}
          <div className="flex items-center gap-1 sm:gap-2 overflow-x-auto pb-1 scrollbar-hide flex-1 mask-linear-fade">
            {/* Root Item */}
            <button
              onClick={() => handleFolderBack()}
              className={`flex items-center gap-2 px-3 py-1.5 sm:py-2 rounded-xl transition-all whitespace-nowrap text-sm sm:text-base ${folderPath.length === 0
                ? 'bg-app-primary/10 text-app-primary font-semibold'
                : 'text-gray-400 hover:text-white hover:bg-app-glassHover'
                }`}
            >
              <Home className="w-4 h-4 sm:w-5 sm:h-5" />
              <span className={folderPath.length > 0 ? 'hidden sm:inline' : ''}>
                {viewMode === 'shared-with-me' ? 'Compartilhados' :
            ...
            {folderPath.map((folder, index) => {
              ...
              return (
                <div key={folder.id} className="flex items-center gap-1 sm:gap-2">
                  ...
                  <button
                    onClick={() => !isLast && handleFolderBack(index)}
                    className={`px-3 py-1.5 sm:py-2 rounded-xl transition-all whitespace-nowrap text-sm sm:text-base max-w-[120px] sm:max-w-[200px] truncate ${isLast
                      ? 'bg-app-primary/10 text-app-primary font-semibold'
                      : 'text-gray-400 hover:text-white hover:bg-app-glassHover'
                      }`}
                    title={folder.name}
        ...
        <p className="text-gray-500 text-xs sm:text-sm hidden sm:block ml-1">
          {searchQuery
            ? `Resultados da busca: "${searchQuery}"`
            : viewMode === 'shared-with-me'
              ? 'Arquivos e pastas compartilhados com você'
              : folderPath.length > 0
                ? `1 item` // Simplified for now, can be dynamic later
      ...
      <div className="lg:hidden px-3 sm:px-6 pb-3">
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="bg-app-surface/50 border border-app-glassBorder rounded-xl px-3 py-1.5 text-sm text-app-textPrimary focus:outline-none focus:ring-2 focus:ring-app-primary transition-shadow"
          >
          ...
          <button
            ...
            className="p-1.5 bg-app-surface/50 border border-app-glassBorder hover:bg-app-glassHover rounded-xl transition-all active:scale-95"
            title={sortDirection === 'asc' ? 'Crescente' : 'Decrescente'}
          >
            {sortDirection === 'asc' ? (
              <ArrowUp className="w-4 h-4 text-app-textSecondary" />
            ) : (
              <ArrowDown className="w-4 h-4 text-app-textSecondary" />
            )}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto">
        {/* Desktop: Tabela */}
        <div key={`desktop-${currentFolderId || 'root'}`} className="hidden lg:block px-3 sm:px-6 animate-slide-up" style={{ animationDuration: '0.4s' }}>
          <div className="glass-panel rounded-2xl overflow-hidden border border-app-glassBorder shadow-glass-sm">
            <table className="w-full">
              <thead>
                <tr className="border-b border-app-glassBorder bg-app-surface/30 backdrop-blur-md">
                  <th className="text-left py-4 px-6 text-sm font-semibold text-app-textSecondary group w-[40%]">
                    <button
                    ...
                    >
                      ...
                      {sortType === 'name' ? (
                        sortDirection === 'asc' ? (
                          ...
                          <ArrowDown className="w-4 h-4 text-app-primary" />
                        )
                      ) : (
                  ...
                  <th className="text-left py-4 px-6 text-sm font-semibold text-app-textSecondary">Proprietário</th>
                  <th className="text-left py-4 px-6 text-sm font-semibold text-app-textSecondary group">
                    <button
        ...
        {/* Mobile: Cards */}
        <div key={`mobile-${currentFolderId || 'root'}`} className="lg:hidden px-3 sm:px-6 pb-4 animate-slide-up" style={{ animationDuration: '0.4s' }}>
          {loading ? (
              ...
              {sortedFiles.map((file) => {
                ...
                return (
                  <div
                    ...
                    className={`
                      glass-panel border border-app-glassBorder rounded-2xl p-3 min-h-[48px]
                      transition-all duration-300 fade-in
                      ${selectedFile?.id === file.id
                        ? 'bg-app-primary/15 border-app-primary/40 shadow-glass-sm ring-1 ring-app-primary/20'
                        : 'hover:bg-app-glassHover active:bg-app-surface/50'
                      }
                    ...
                    <div className="flex items-center gap-2 sm:gap-3" style={{ overflow: 'visible', width: '100%', position: 'relative' }}>
                      ...
                      {mobileMenuOpen === file.id && !mobileColorPicker && (
                        <>
                          ...
                          <div
                            data-context-menu
                            className="fixed bottom-20 left-4 right-4 lg:absolute lg:right-0 lg:top-full lg:bottom-auto lg:left-auto lg:mt-2 lg:w-64 glass-panel border border-app-glassBorder rounded-2xl shadow-glass z-[106]"
                            onClick={(e) => {
                              ...
                              {file.type === 'folder' ? (
                                <>
                                  ...
                                  <button
                                    ...
                                    className="w-full px-4 py-3 text-left text-sm text-gray-300 hover:bg-gray-800 active:bg-gray-700 rounded-lg transition-all duration-300 active:scale-[0.98] flex items-center gap-3 touch-manipulation mb-2"
                                ...
                                <>
                                  ...
                                  <button
                                    ...
                                    className="w-full px-4 py-3 text-left text-sm text-gray-300 hover:bg-gray-800 active:bg-gray-700 rounded-lg transition-all duration-300 active:scale-[0.98] flex items-center gap-3 touch-manipulation disabled:opacity-50 disabled:cursor-not-allowed"
                                    style={{ pointerEvents: 'auto', WebkitTapHighlightColor: 'transparent' }}
                                  >
                          ...
                          <div className="grid grid-cols-5 gap-3">
                            {FILE_COLOR_OPTIONS.map((color) => (
                              <button
                                ...
                                className={`w-10 h-10 rounded-lg border-2 transition-transform active:scale-95 focus:outline-none touch-manipulation ${(file.type === 'folder' ? getFolderColor(file.id) : getFileTextColor(file.id)) === color ? 'border-white' : 'border-transparent'
                                  }`}
