              ...
              <tbody>
                {loading ? (
                  ...
                  filteredFiles.map((file, index) => {
                    ...
                    // Debug: logs para arquivos com anotações e também para vídeos que deveriam ter anotações
                    if (hasAnnots || folderHasAnnotatedVideos || (isVideo && filesWithAnnotations.size > 0)) {
                      console.log('[FileList] Desktop: Detectando anotações para:', {
                        nome: file.name,
                        id: file.id,
                        tipo: file.type,
                        ...
                        isVideo,
                        hasAnnots,
                        folderHasAnnotatedVideos,
                        noSet: filesWithAnnotations.has(file.id),
                        noFoldersSet: foldersWithAnnotatedVideos.has(file.id),
                        filesWithAnnotationsSize: filesWithAnnotations.size,
                        foldersWithAnnotatedVideosSize: foldersWithAnnotatedVideos.size
                      });
                    }
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {filteredFiles.map((file, index) => {
                ...
                // Debug: logs para arquivos com anotações e também para vídeos que deveriam ter anotações
                if (hasAnnots || folderHasAnnotatedVideos || (isVideo && filesWithAnnotations.size > 0)) {
                  console.log('[FileList] Mobile: Detectando anotações para:', {
                    nome: file.name,
                    id: file.id,
                    tipo: file.type,
                    extension: file.extension,
